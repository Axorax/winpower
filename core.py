...
def get_power_mode():
    try:
        output = subprocess.check_output(
            ["powercfg", "/getactivescheme"], encoding="utf-8"
        )
...
def set_power_mode(mode):
    if mode in power_schemes:
        try:
            subprocess.check_call(["powercfg", "/setactive", power_schemes[mode]])
...
def get_powercfg_value(subgroup_guid, setting_guid):
    ...
    ac_value = int(match_ac.group(1), 16) if match_ac else "null"
    dc_value = int(match_dc.group(1), 16) if match_dc else "null"
...
def get_timeouts():
    def pretty(value):
        rounded_value = round(value / 60, 1) if value else 0
...
def disable_hibernation():
    try:
        subprocess.check_call(["powercfg", "/hibernate", "off"])
...
def get_hibernation():
    try:
        output = subprocess.check_output(["powercfg", "/a"], encoding="utf-8")
        return (
...
def generate_battery_report():
    try:
        subprocess.check_call(["powercfg", "/batteryreport"], shell=True)
        return True
    except subprocess.CalledProcessError:
        return False
...
def generate_sleepstudy_report():
    try:
        subprocess.check_call(["powercfg", "/sleepstudy"], shell=True)
        return True
    except subprocess.CalledProcessError:
        return False
...
def get_brightness():
    try:
        ...
        print(f"An error occurred while getting brightness: {e}")
        return None
...
def set_brightness(brightness_value):
    try:
        ...
        print(f"An error occurred while setting brightness: {e}")
